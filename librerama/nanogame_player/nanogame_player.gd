...
func _physics_process(delta: float) -> void:
	...
	if is_snapping_allowed and not is_inside_body:
		...
		for i: int in collisions.size():
			...
			var result: Array[Dictionary] =\
					_physics_space.intersect_point(_physics_point_query, 1)
